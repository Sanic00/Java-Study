...
public class ArrayTest {
	...
	public static void main(String[] args) throws IOException{ // 추후 예외처리 기능으로 throws IOException 설정가능
		
		BufferedReader br= new BufferedReader(new InputStreamReader(System.in));
		
		String name = "" ; //이름 초기화
		int[]jumin = new int [14]; //주민번호 입력배열 (7번째 자리는 '-'표시 이므로 계산 되지 않음)
		boolean right = false ; //주민번호가 맞는지 틀린지 판별하기위해 설정.
		...
		do {
			right = false ;
			System.out.print("주민번호 입력(ex:111111-111111)");
			...
			int hap = 0, cre = 2; // Q.cre가 나타나는 것이 무엇입니까? A. 곱하기 값
	 		float temp = 0.0f, temp1 = 0.0f ;
			
			for(int i =0 ; i<jumin.length-1 ;i++) {// 13개?
				if(i==6) {
					continue ;
				}
				hap += jumin[i]*cre;
